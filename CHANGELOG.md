...
- **Script fixes.** Scripts no longer overwrite each other's settings or run over each other, a typo can no longer freeze script loading, and a variable at the end of a sentence now shows its value.
- **Chat history reworked.** History from the server now appears in order below your saved scrollback, without repeating your own messages, joins, parts or quits. Rejoining a channel fetches what you missed while away, and any stretch that couldn't be fetched is marked.
- **Channel greetings show on every join.**
- **Reactions appear on the message.** A reaction to a message that isn't loaded fetches that message instead of showing a message ID.
- **Faster scrollback loading** for long-running channels.
- **The member list stays accurate** while history is replayed.
- **Safer file uploads.** Your password only goes to an upload server that belongs to the same network, failed uploads say why, and the link is kept if you switch channels mid-upload.
- **The network editor uses the same layout as Settings** on phones.
- **All WHOIS replies appear where you ran WHOIS.**
- **`/ctcp` sends your text as typed.**
- **Fixed:** new aliases not saving, coloured text art breaking apart, and private messages sometimes opening a window for your own nick.
- **Fixed:** a wrong phone clock affecting which messages count as new, and the read marker jumping backwards.
- **Fixed:** a crash when long-pressing some lines, a crash while loading scrollback, a stall during log cleanup, and resources left behind after closing the app.
