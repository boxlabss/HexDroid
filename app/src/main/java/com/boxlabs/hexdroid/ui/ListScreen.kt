...
fun ListScreen(
    ...
    Scaffold(
        ...
        Column(Modifier.fillMaxSize().padding(padding).padding(12.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            OutlinedTextField(value = state.listFilter, onValueChange = onFilterChange, label = { Text(stringResource(R.string.list_search_hint)) }, singleLine = true, modifier = Modifier.fillMaxWidth())
            Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                if (state.listInProgress) { CircularProgressIndicator(modifier = Modifier.size(18.dp), strokeWidth = 2.dp); Text(stringResource(R.string.list_loading, state.channelDirectory.size), style = MaterialTheme.typography.bodySmall) }
                ...
                items(items.take(3000)) { ch ->
                    Column(Modifier.fillMaxWidth().clickable { onJoin(ch.channel) }.padding(vertical = 10.dp)) {
                        Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                            ...
                            Text(stringResource(R.string.list_join_button), style = MaterialTheme.typography.bodySmall)
                        }
